...
function openItemDomainosList(item,listId){
    var x ='dominosDropdown'+listId;
    var xy ='#dominosDropdown'+listId;
    $(item).attr('data-activates',x);
    $(item).dropdown('open');
}
...
if (hasCSSClipPaths) {$html.addClass('cssclippaths');}
$(window).scrollTop(0);
...
function lockScroll(pointer) {
    ...
    if (w2 > w1) {$body.css('padding-right', w2 - w1);}
}

function unlockScroll() {$body.css('padding-right', '').removeClass('js-lock-scroll js-lock-pointer');}
...
$wrapper.on('click', function(e) {
    ...
    if ($target.parents('.modal').length && $target.data('action') !== 'close') {
        if (!$target.parents('[data-action="close"]').length) {return;}
    }
    e.preventDefault();
    requestAnimationFrame(function() {closeOverlay();});
});
